...
const ensureIdentifierStart = (value: string) => {
  if (!value) return ''
...
const toCamel = (value: string) => {
  ...
  const raw = [first ?? 'node', ...rest.map((word) => word[0].toUpperCase() + word.slice(1))].join(
    '',
  )
  return ensureIdentifierStart(raw)
}
...
const buildNodeNames = (nodes: Node<FlowNodeData>[]) => {
  ...
  nodes.forEach((node, index) => {
    ...
    const labelKey = ensureIdentifierStart(toSnake(node.data.label))
    const base = labelKey || `${roleSeed}_${index + 1}`
    let key = base
    let counter = 1
    while (used.has(key)) {
      counter += 1
...
export const validateGraph = ({ nodes, edges }: GraphSnapshot): ValidationIssue[] => {
  ...
  edges.forEach((edge) => {
    if (!edge.data?.kind) {
      issues.push({
        level: 'warning',
        message: `Edge ${edge.id} is missing a kind and will be treated as normal.`,
      })
    }
    if (edge.data?.kind === 'conditional' && !edge.data.routeKey?.trim()) {
      issues.push({
        level: 'warning',
        message: `Conditional edge ${edge.id} is missing a route key.`,
      })
    }
  })

  nodes.forEach((node) => {
    if (node.data.roles.length === 0) {
      issues.push({
        level: 'warning',
        message: `Node "${node.data.label || node.id}" has no roles assigned.`,
      })
    }
  })
...
export const generateLangGraphPython = ({ nodes, edges }: GraphSnapshot) => {
  ...
  lines.push('# Generated by AgentBuilder')
